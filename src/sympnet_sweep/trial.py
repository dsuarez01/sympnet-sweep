...
def build_trial_fn(args: argparse.Namespace) -> Callable[[dict], None]:

	def run_trial(config: dict) -> None:

		trial_config = TrialConfig(**config["config"])
		...
		wandb_config = {k:v for k,v in vars(trial_config).items() if k != "checkpt"}
